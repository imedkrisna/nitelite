...
ARDL on the quarterly real GDP and nightlight data perform the best out of the models tested. The VECM of both growth and GDP data do not really use the night light index, and relies solely on its own lag. Meaning, VECM cannot be used if one wants to use the nightlight index as a leading indicators.

In the next section, we try to use the nightlight index as a leading indicator for GDP where we divide sets of data into training and testing data.
