...
This paper runs two types of analysis. First, we utilise Indonesia's night light index to fit quarterly GDP growth rate. The nature of the time series data leads us to use Autoregressive Distributed Lag (ARDL) modes. Using this approach, we find , Vector Error Correction Model (VECM) and Vector AutoRegression (VAR) models. Second, we use logged quarterly GDP and night light index to fit the level of GDP instead of growth rate. This is done as a robustness check. VAR is not used in this case since the two series are not stationary when the data is level.
