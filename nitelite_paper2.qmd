...
This paper aims to test whether nighttime light can be a sole predictor of GDP without adding other variables. Therefore, this paper aims to be as simple as possible. We find that for the national level data, ARDL provides excellent estimators to forecast GDP.
...
![Economic growth prediction with ARDL](fig/ARDL.png){#fig-4}

@fig-4 shows the ARDL approach. This time, we see the panel (c) as the best performing one, representing the model with a scarring dummy and no quarterly dummy. The panel (c) uses $ARDL(p,q,r)=ARDL(7,8,5)$ specification where $p,q,r$ and lags for GDP growth, night light growth and scarring dummy respectively. Again, we find the night light to be a less then ideal predictor, where only lag 6 is significant at 10% level. However, this equation seems to be the better performing one so far.

![Economic growth prediction with ARDL](fig/ARDL_train_test_forecast.png){#fig-5}

We split the observation in 2024 to use the 
