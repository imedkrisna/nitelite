...
There are two potential key takeaways from the national-level analysis. First, the scarring effect is important to forecast GDP, more than other variables. We can see this especially when we use quarterly dummy. The panel (c) and (f) show that quarterly dummy do not significantly add insight, while panel (d) and (e), panels with quarterly dummy without scarring, show a looser fit. This results corroborates nighttime light index can be used to predict GDP growth well. Second, there is a potential structural break after COVID-19, which suggests that the relationship between nighttime light and GDP has changed. This finding is consistent with other studies that find the potential scarring effect from the COVID pandemic in Indonesia [@pangestu].
